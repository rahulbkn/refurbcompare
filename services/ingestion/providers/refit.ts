...
export const refitAdapter: ProviderAdapter = {
  slug: "refit",
  label: "Refit",
  ...
  async fetchListings(): Promise<FeedListing[]> {
    if (!DEMO_GATED) {
      throw new Error(
        "refit adapter is a stub: no authorized feed configured (see PROVIDER_INTEGRATION.md). TEST fixtures only with NEXT_PUBLIC_DEMO_MODE=true.",
      );
    }
    return buildDemoListingsForSeller("refit");
  },
};