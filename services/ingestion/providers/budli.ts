...
export const budliAdapter: ProviderAdapter = {
  slug: "budli",
  label: "Budli",
  ...
  async fetchListings(): Promise<FeedListing[]> {
    if (!DEMO_GATED) {
      throw new Error(
        "budli adapter is a stub: no authorized feed configured (see PROVIDER_INTEGRATION.md). TEST fixtures only with NEXT_PUBLIC_DEMO_MODE=true.",
      );
    }
    return buildDemoListingsForSeller("budli");
  },
};