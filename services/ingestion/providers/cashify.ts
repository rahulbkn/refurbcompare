...
export const cashifyAdapter: ProviderAdapter = {
  ...
  async fetchListings(): Promise<FeedListing[]> {
    if (!DEMO_GATED) {
      throw new Error(
        "cashify adapter is a stub: no authorized feed configured (see PROVIDER_INTEGRATION.md). TEST fixtures only with NEXT_PUBLIC_DEMO_MODE=true.",
      );
    }
    return buildDemoListingsForSeller("cashify");
  },
};